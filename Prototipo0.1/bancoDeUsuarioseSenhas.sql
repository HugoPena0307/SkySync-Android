...
create table automovel (
	id_automovel int primary key identity,
	placa char(7),
	id_login int,
	constraint fk_PesCarro foreign key (id_login) references login(id_login)
)
...
create table agendamento (
	...
	id_usuario int,
	data date,
	constraint fk_AgendaAutomovel foreign key (id_automovel) references automovel(id_automovel)
	constraint fk_AgendaUsuario foreign key (id_usuario) references login(id_login)
)